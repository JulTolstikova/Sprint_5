...
class Test_Profile:
    def test_transition_to_profile(self, driver, register_user):
        ...
        # Проверяем, что отображается заголовок Профиль
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located(TestLocators.PROFILE_TITLE))

        # Проверяем, что в профиле корректно отображается почта
        assert driver.find_element(*TestLocators.INPUT_NAME).get_attribute("value") == register_user["email"]
